...
const closeElementBtn = document.getElementById('closeElementBtn');
...
function setupEventListeners() {
  ...
  playAgainBtn.addEventListener('click', () => {
    handlePlayAgainFromCompleteModal();
  });
  ...
  [historyModal, leaderboardModal, achievementsModal, infoModal, donateModal, elementModal].forEach(modal => {
    modal.querySelector('.modal-overlay')?.addEventListener('click', () => closeModal(modal));
  });
  completeModal.querySelector('.modal-overlay')?.addEventListener('click', () => dismissCompleteModal());
  closeElementBtn.addEventListener('click', () => closeModal(elementModal));
...
function handleElementClick(e) {
  ...
  // In View Mode, every cell click reveals full element properties
  if (viewTableBtn.classList.contains('viewing')) {
    openElementModal(parseInt(element.dataset.atomic));
    return;
  }
...
function validateInput(element, userInput) {
  ...
  if (isCorrect) {
    ...
    recordElementMastery(atomic, true);
    element.classList.remove('active');
    if (input) input.remove();
    ...
    recordElementMastery(atomic, false);
...
function resetChallenge() {
  ...
  periodicTable.classList.remove('view-mode');
...
// Element Mastery Tracking (per-element learning progress)
const MASTERY_STORAGE_KEY = 'elementMastery';
let _masteryCache = null;

function loadMasteryData() {
  if (_masteryCache) return _masteryCache;
  try {
    const raw = safeGetItem(MASTERY_STORAGE_KEY);
    _masteryCache = raw ? JSON.parse(raw) : {};
  } catch {
    _masteryCache = {};
  }
...
function saveMasteryData() {
  if (_masteryCache) safeSetItem(MASTERY_STORAGE_KEY, JSON.stringify(_masteryCache));
}

function recordElementMastery(atomic, isCorrect) {
  const data = loadMasteryData();
  const rec = data[atomic] || { correct: 0, wrong: 0, lastSeen: null };
...
function getElementMastery(atomic) {
  const rec = loadMasteryData()[atomic];
  if (!rec) return null;
  const total = rec.correct + rec.wrong;
  const accuracy = total > 0 ? Math.round((rec.correct / total) * 100) : 0;
  // Mastery favors accuracy with a volume cushion: more correct reps = higher confidence
  const reps = Math.min(rec.correct, 10);
  const masteryPct = Math.round(accuracy * 0.7 + reps * 3);
  return {
    accuracy,
    reviews: total,
    masteryPct: Math.min(masteryPct, 100),
...
function dismissCompleteModal() {
  closeModal(completeModal);
  // Closing the complete modal (X, overlay click, or Escape) keeps the
  // finished table exactly as the user left it — correct/incorrect cells,
  // typed answers, everything. Only "Play Again" should reset the table.
  revealFinishedTable();
...
// After a finished (but not reset) challenge, fully reveal the table —
// correct cells stay green, incorrect stay red (Recall mode), everything
// else shows its symbol/name in the default style — and let any cell be
// clicked to open its full property card.
function revealFinishedTable() {
  viewTableBtn.classList.add('viewing');
  periodicTable.classList.add('view-mode');
  viewTableBtn.querySelector('i').className = 'ri-eye-off-line';
  ...
  document.querySelectorAll('.element').forEach(el => {
    ...
    if (!symbolSpan.textContent) symbolSpan.textContent = el.dataset.symbol;
    if (!nameSpan.textContent) nameSpan.textContent = el.dataset.name;
  });
}

function handlePlayAgainFromCompleteModal() {
  closeModal(completeModal);
  const shouldShowRecallGuide = state.totalChallengesCompleted >= 3
    && state.gameMode === 'classic'
    && !safeGetItem('recallModeGuideShown');
...
function openElementModal(atomicNumber) {
  ...
  if (!el) return;
  const detail = (typeof ELEMENT_DETAILS !== 'undefined' && ELEMENT_DETAILS[atomicNumber]) || {};
  const accent = getCategoryColor(el.category);

  // Header
  document.getElementById('emNumber').textContent = el.atomicNumber;
  document.getElementById('emSymbol').textContent = el.symbol;
  document.getElementById('emName').textContent = el.name;

  const catEl = document.getElementById('emCategory');
  catEl.textContent = capitalizeWords(el.category);
  catEl.style.color = accent;
  catEl.style.background = accent.replace('var(', 'color-mix(in srgb, var(').replace(')', ') 18%, transparent)');
  catEl.style.borderColor = accent.replace('var(', 'color-mix(in srgb, var(').replace(')', ') 45%, transparent)');

  const occurrence = getOccurrence(atomicNumber, detail.discoveredBy);
  const occEl = document.getElementById('emOccurrence');
  occEl.textContent = occurrence;
  occEl.className = `element-modal-occurrence em-occ-${occurrence.toLowerCase()}`;

  document.getElementById('emSymbol').style.color = accent;
  document.querySelector('.element-modal-symbol-block').style.borderColor = accent;

  // Learning progress
  const mastery = getElementMastery(atomicNumber);
  const progressSection = document.getElementById('emProgressSection');
  if (mastery) {
    progressSection.classList.remove('hidden');
    document.getElementById('emMasteryBar').style.width = `${mastery.masteryPct}%`;
    document.getElementById('emMasteryBar').style.background = accent;
    document.getElementById('emMasteryPct').textContent = `${mastery.masteryPct}% mastered`;
    document.getElementById('emAccuracy').textContent = `${mastery.accuracy}%`;
    document.getElementById('emReviews').textContent = mastery.reviews;
    document.getElementById('emLastSeen').textContent = formatRelativeTime(mastery.lastSeen);
  } else {
    progressSection.classList.add('hidden');
  }

  // Mass shown inline inside the element cell, like a real periodic table tile
  const massBig = document.getElementById('emMassBig');
  massBig.textContent = formatMass(detail.mass);
  massBig.title = detail.mass != null ? `${detail.mass} u (exact)` : '';

  // Position
  document.getElementById('emPositionGrid').innerHTML = renderCards([
    { label: 'Period', value: el.period ?? '—' },
    { label: 'Group', value: el.group ?? '—' },
    { label: 'Block', value: el.block ? `${el.block}-block` : '—' }
  ]);

  // Electronic structure
  document.getElementById('emEconfigBox').innerHTML = renderEconfigPills(detail.econfig);
  document.getElementById('emElectronicGrid').innerHTML = renderCards([
    { label: 'Valence', value: detail.valence ?? '—' },
    { label: 'Electronegativity', value: detail.electroneg != null ? detail.electroneg : '—' }
  ]);

  // Physical properties
  document.getElementById('emPhysicalGrid').innerHTML = renderCards([
    { label: 'Phase (STP)', value: detail.phase || '—' },
    { label: 'Density', value: detail.density != null ? `${detail.density} g/cm³` : '—' },
    { label: 'Melting Point', value: kelvinToDisplay(detail.melt) },
    { label: 'Boiling Point', value: kelvinToDisplay(detail.boil) }
  ]);

  // Discovery
  document.getElementById('emDiscoveryText').textContent = formatDiscoveryText(detail.discoveredBy);

  const appearanceEl = document.getElementById('emAppearance');
  const appearanceWrap = document.getElementById('emAppearanceWrap');
  const discoveryGrid = document.getElementById('emDiscoveryGrid');
  if (detail.appearance) {
    appearanceEl.textContent = capitalizeWords(detail.appearance, true);
    appearanceWrap.classList.remove('hidden');
    discoveryGrid.classList.remove('em-card-grid-single');
  } else {
    appearanceEl.textContent = '';
    appearanceWrap.classList.add('hidden');
    discoveryGrid.classList.add('em-card-grid-single');
  }

  // Wikipedia link
  const wikiLink = document.getElementById('emWikiLink');
  if (detail.wiki) {
    wikiLink.href = detail.wiki;
...
function formatMass(mass) {
  if (mass == null) return '—';
  // Round to a student-friendly precision while staying accurate.
  // Standard periodic table tiles show the bare number, no unit suffix.
  const rounded = Math.round(mass * 100) / 100;
  return `${rounded}`;
}

// Standard, well-established chemistry facts (not per-element trivia):
// elements heavier than Bismuth are radioactive, Tc/Pm are the two light
// exceptions, and anything with no naturally occurring isotopes is man-made.
const SYNTHETIC_INSTITUTION_HINTS = ['Berkeley', 'Joint Institute', 'GSI', 'Gesellschaft', 'Dubna', 'RIKEN'];
function getOccurrence(atomicNumber, discoveredBy) {
  const isSynthetic = atomicNumber >= 95
    || (discoveredBy && SYNTHETIC_INSTITUTION_HINTS.some(h => discoveredBy.includes(h)) && atomicNumber > 92);
  if (isSynthetic) return 'Synthetic';
  if (atomicNumber >= 84 || atomicNumber === 43 || atomicNumber === 61) return 'Radioactive';
  return 'Native';
}

function formatDiscoveryText(discoveredBy) {
  if (!discoveredBy) return 'Known since antiquity';
  const isPlaceOrDate = /\d|bc|ancient|unknown/i.test(discoveredBy)
    || ['Middle East', 'India', 'China', 'Persia', 'Egypt', 'Phoenicia'].includes(discoveredBy);
  return isPlaceOrDate ? `Known since: ${discoveredBy}` : `Discovered by ${discoveredBy}`;
}

const EM_ORBITAL_COLORS = { s: '#3b82f6', p: '#8b5cf6', d: '#06b6d4', f: '#f59e0b' };

function renderEconfigPills(econfig) {
  if (!econfig) return '—';
  return econfig.split(' ').map(token => {
    const orbitalMatch = token.match(/^(\d+)([spdf])(\d+)$/);
    if (orbitalMatch) {
      const [, n, letter, exp] = orbitalMatch;
      const color = EM_ORBITAL_COLORS[letter];
      return `<span class="em-econfig-pill" style="--pill-color:${color}">${n}${letter}<sup>${exp}</sup></span>`;
    }
    // Noble-gas core notation, e.g. "[Xe]" or "*[Rn]"
    return `<span class="em-econfig-core">${token}</span>`;
  }).join('');
}
...
function renderCards(items) {
  return items.map(p => `
    <div class="em-card">
      <span class="em-card-label">${p.label}</span>
      <span class="em-card-value">${p.value}</span>
    </div>
  `).join('');
}

function kelvinToDisplay(k) {
  if (k == null) return '—';
  return `${k} K (${Math.round(k - 273.15)}°C)`;
}

function capitalizeWords(str, sentenceOnly = false) {
  if (!str) return '';
  if (sentenceOnly) return str.charAt(0).toUpperCase() + str.slice(1);
...
function toggleViewTable() {
  const challengeFinished = finishBtn.classList.contains('play-again');
  // Block reveal while a challenge is in progress; allow hide toggle when already viewing
  // or when the challenge has already finished (Play Again is showing).
  if (state.totalAttempts > 0 && state.correctElements.size < state.activeElements.size
      && !viewTableBtn.classList.contains('viewing') && !challengeFinished) {
  ...
  showHintToast('Tap any element to see its full properties');
